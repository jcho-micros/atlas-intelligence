...
class OpportunityService:
    ...
    def calculate_for_keyword(self, keyword: Keyword) -> Opportunity:
        ...
        personalization_score = round((personalized_count / listing_count) * 100, 2) if listing_count else 0.0
        ...
        raw_score = demand_score + profit_score + (personalization_score * 0.7) - (competition_score * 0.8)
        score = round(max(0, min(100, raw_score)), 2)
        recommendation = "BUILD" if score >= 80 else "WATCH" if score >= 55 else "WAIT"
        notes = f"{listing_count} listings analyzed; avg price ${avg_price}; {personalized_count} personalized; {digital_count} digital"
