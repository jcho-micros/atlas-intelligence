...
class ListingService:
    ...
    def save_listings(self, keyword: Keyword, marketplace: str, listings: list[MarketplaceListing]) -> list[Listing]:
        saved_or_updated: list[Listing] = []
        for item in listings:
            shop = self._get_or_create_shop(item, marketplace)
            existing = self.session.query(Listing).filter_by(
                keyword_id=keyword.id, marketplace=marketplace, external_id=item.external_id
            ).first()
            if existing:
                ...
                listing = Listing(
                    keyword_id=keyword.id, shop_id=shop.id, marketplace=marketplace, external_id=item.external_id,
                    title=item.title, price=item.price, currency=item.currency, shop_name=item.shop_name,
                    review_count=item.review_count, rating=item.rating, url=item.url, image_url=item.image_url,
                    is_personalized=item.is_personalized, is_digital=item.is_digital,
                    shipping_price=item.shipping_price, processing_time=item.processing_time,
                    num_favorers=item.num_favorers, views=item.views, quantity=item.quantity,
                    tags=item.tags, materials=item.materials, processing_min=item.processing_min,
                    processing_max=item.processing_max, created_timestamp=item.created_timestamp,
                    updated_timestamp=item.updated_timestamp,
                )
                self.session.add(listing)
                self.session.flush()
            saved_or_updated.append(listing)
        self.session.commit()
        return saved_or_updated

    def _update_listing(self, listing: Listing, item: MarketplaceListing, shop_id: int) -> None:
